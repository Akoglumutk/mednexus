...
export default function TrialsEditor() {
    ...
    lines.forEach(line => {
      ...
      if (answerMatch) {
        ...
        if (currentMode === 'question') {
          ...
          else questionText = line.replace(/^\d+[\s.)\-\/]+/, '');
        } else if (currentMode === 'options') {
          if (parsedOptions.length > 0) parsedOptions[parsedOptions.length - 1] += '\n' + line;
  ...
  const handlePromptConfirm = async () => {
    ...
    if (action === 'cancel') {
      if (id === 'new') router.push('/trials');
      else router.push(`/trials/${id}`);
    } 
  ...
  return (
    <main onPaste={async (e) => {
      ...
      <DivinePrompt 
        ...
        onCancel={() => setPromptState({ isOpen: false, title: '', actionType: null })}
      />
    </main>
  );
}
